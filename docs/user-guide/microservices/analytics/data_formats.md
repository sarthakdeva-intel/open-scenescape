...
[Scene Controller Data Formats](../controller/data_formats.md#data-scene-output-message-format))
...
[Data Scene Output Message Format](../controller/data_formats.md#data-scene-output-message-format)
and [Common Output Track Fields](../controller/data_formats.md#common-output-track-fields)
...
- **Badge / access-control sensors** — `value` holds a string badge identifier (e.g.
  `"BADGE-00421"`), allowing the Analytics service to associate a personnel ID with an object
  track inside the sensor's measurement area.
...
The Analytics service publishes regulated (rate-controlled) tracking results aggregating all
